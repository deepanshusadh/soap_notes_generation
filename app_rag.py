...
if st.session_state.soap_note==1:
    ...
    if input_prompt:
        ...
        st.session_state.response_retrieve= f"{soap_notes.retrieve_response(input_prompt)}"

        #generat soap notes
        response_gen= f"{soap_notes.generate_response(input_prompt,st.session_state.response_retrieve,st.session_state.config_id)}"
...
# elif soap note flag is 0, start the chat version keeping generated soap note as context.
    ...
    if input_prompt :
        ...
        #chat
        response_chat= f"{soap_notes.chat(input_prompt,st.session_state.response_retrieve,st.session_state.config_id)}"
        with st.chat_message("assistant"):
